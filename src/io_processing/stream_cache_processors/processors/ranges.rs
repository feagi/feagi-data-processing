...
impl StreamCacheProcessor for LinearScaleTo0And1 {
    ...
    fn process_new_input(&mut self, value: &IOTypeData) -> Result<&IOTypeData, FeagiDataProcessingError> {
        ...
        if float_result.is_err() {
            return Err(IODataError::InvalidParameters("Value for LinearScaleTo0And1 must be a F32!".into()).into());
        }
        
        let clamped = float_result.unwrap().clamp(self.lower, self.upper);
