...
pub trait NeuronTranslator<T> {
    ...
    fn generate_neuron_data_single_channel(&self, value: T, target_to_overwrite: &mut NeuronXYZPArrays, channel: ChannelIndex) -> Result<(), DataProcessingError>;

    fn generate_neuron_data_multi_channel(&self, channels_and_values: HashMap<ChannelIndex, T>, target_to_overwrite: &mut NeuronXYZPArrays) -> Result<(), DataProcessingError>;
}